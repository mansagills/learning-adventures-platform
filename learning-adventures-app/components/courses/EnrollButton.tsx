    ...
    try {
      ...
      if (data.success) {
        ...
        if (details?.prerequisitesMet === false) {
          ...
          setError('This course requires a premium subscription. Upgrade to access!');
        } else if (details?.freeCourseLimit && details?.freeCoursesEnrolled >= details.freeCourseLimit) {
          setError(
            `Free users can only enroll in ${details.freeCourseLimit} courses. Upgrade to premium for unlimited access!`
          );
        } else {
