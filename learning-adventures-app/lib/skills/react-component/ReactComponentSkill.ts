...
export class ReactComponentSkill extends BaseSkill {
  public getMetadata(): SkillMetadata {
    return {
      ...
      examples: [
        'Create a React game for multiplication',
        'Build a React component for fractions',
      ],
      version: '1.0.0',
  ...
  public async canHandle(
    ...
    if (userRequest.toLowerCase().includes('react'))
    ...
    try {
      ...
      if (!concept) {
        return this.buildErrorResult(
          'Could not determine game specifications',
          'MISSING_CONCEPT'
        );
      }
      ...
      return this.buildSuccessResult(
        ...
        Date.now() - startTime,
        90,
        ['accessibility-validator', 'metadata-formatter']
      );
    } catch (error) {
      return this.buildErrorResult(
  ...
  protected validate(code: string): boolean {
    return (
      code.includes('export') && code.includes('function') && code.length > 100
    );
  }
