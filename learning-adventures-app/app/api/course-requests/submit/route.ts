...
function validateCourseRequest(data: any): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (!data.studentAge || data.studentAge < 1) errors.push('Valid student age is required');
  ...
  // Section 10: Consent
  if (!data.consentGiven) errors.push('You must provide consent to submit the request');
