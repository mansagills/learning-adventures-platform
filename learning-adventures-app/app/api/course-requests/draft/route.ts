...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user || (user.role !== 'PARENT' && user.role !== 'TEACHER' && user.role !== 'ADMIN')) {
      return NextResponse.json({ error: 'Only parents and teachers can create course requests' }, { status: 403 });
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (existing.userId !== session.user.id) {
      return NextResponse.json({ error: 'Unauthorized to update this draft' }, { status: 403 });
    }

    // Conflict detection: check if server version is newer
    if (updateData.clientLastSaved && existing.updatedAt > new Date(updateData.clientLastSaved)) {
      return NextResponse.json({
        conflict: true,
    ...
    return NextResponse.json(
      { error: 'Failed to update draft', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
