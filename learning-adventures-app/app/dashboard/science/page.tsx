...
function ScienceDashboardContent() {
  ...
  const scienceAdventures = allAdventures.filter(adv => adv.category === 'science');

  const scienceProgress = progressData?.progress.filter(p => {
    const adventure = scienceAdventures.find(a => a.id === p.adventureId);
    return !!adventure;
  }) || [];

  const completedScience = scienceProgress.filter(p => p.status === 'COMPLETED');
  const inProgressScience = scienceProgress.filter(p => p.status === 'IN_PROGRESS');

  const averageScore = completedScience.length > 0
    ? Math.round(completedScience.reduce((sum, p) => sum + (p.score || 0), 0) / completedScience.length)
    : 0;

  const totalTimeSpent = scienceProgress.reduce((sum, p) => sum + (p.timeSpent || 0), 0);
  ...
  const scienceSkills: Skill[] = [
    {
      ...
      progress: Math.min(100, (completedScience.filter(p => {
        const adv = scienceAdventures.find(a => a.id === p.adventureId);
        return adv?.skills.some(s => s.toLowerCase().includes('biology') || s.toLowerCase().includes('life'));
      }).length / Math.max(1, scienceAdventures.filter(a =>
        a.skills.some(s => s.toLowerCase().includes('biology') || s.toLowerCase().includes('life'))
      ).length)) * 100),
    ...
    },
    {
      ...
      progress: Math.min(100, (completedScience.filter(p => {
        const adv = scienceAdventures.find(a => a.id === p.adventureId);
        return adv?.skills.some(s => s.toLowerCase().includes('chemistry') || s.toLowerCase().includes('matter'));
      }).length / Math.max(1, scienceAdventures.filter(a =>
        a.skills.some(s => s.toLowerCase().includes('chemistry') || s.toLowerCase().includes('matter'))
      ).length)) * 100),
      level: 'beginner',
      completedActivities: completedScience.filter(p => {
        const adv = scienceAdventures.find(a => a.id === p.adventureId);
    ...
    },
    {
      ...
      progress: Math.min(100, (completedScience.filter(p => {
        const adv = scienceAdventures.find(a => a.id === p.adventureId);
        return adv?.skills.some(s => s.toLowerCase().includes('physics') || s.toLowerCase().includes('energy'));
      }).length / Math.max(1, scienceAdventures.filter(a =>
        a.skills.some(s => s.toLowerCase().includes('physics') || s.toLowerCase().includes('energy'))
      ).length)) * 100),
      level: 'beginner',
      completedActivities: completedScience.filter(p => {
        const adv = scienceAdventures.find(a => a.id === p.adventureId);
    ...
    },
    {
      ...
      progress: Math.min(100, (completedScience.filter(p => {
        const adv = scienceAdventures.find(a => a.id === p.adventureId);
        return adv?.skills.some(s => s.toLowerCase().includes('earth') || s.toLowerCase().includes('space'));
      }).length / Math.max(1, scienceAdventures.filter(a =>
        a.skills.some(s => s.toLowerCase().includes('earth') || s.toLowerCase().includes('space'))
      ).length)) * 100),
      level: 'beginner',
      completedActivities: completedScience.filter(p => {
        const adv = scienceAdventures.find(a => a.id === p.adventureId);
    ...
    },
    {
      ...
      progress: Math.min(100, (completedScience.length / Math.max(1, scienceAdventures.length)) * 100),
  ...
  const learningPath: PathNode[] = [
    ...
    },
    {
      ...
      type: completedScience.length >= 3 ? (completedScience.length >= 5 ? 'completed' : 'current') : (completedScience.length > 0 ? 'available' : 'locked'),
    ...
    },
    {
      ...
      type: completedScience.length >= 5 ? (completedScience.length >= 8 ? 'completed' : 'current') : (completedScience.length >= 3 ? 'available' : 'locked'),
      skills: ['Biology', 'Ecosystems', 'Life Cycles'],
      estimatedTime: 35,
      adventureId: scienceAdventures.find(a => a.skills.some(s => s.toLowerCase().includes('biology')))?.id,
    ...
    },
    {
      ...
      type: completedScience.length >= 8 ? (completedScience.length >= 12 ? 'completed' : 'current') : (completedScience.length >= 5 ? 'available' : 'locked'),
      skills: ['Chemistry', 'Physics', 'Energy'],
      estimatedTime: 45,
      adventureId: scienceAdventures.find(a => a.skills.some(s => s.toLowerCase().includes('chemistry')))?.id,
    ...
    },
    {
      ...
      type: completedScience.length >= 12 ? 'current' : (completedScience.length >= 8 ? 'available' : 'locked'),
