...
function MathDashboardContent() {
  ...
  const averageScore =
    completedMath.length > 0
      ? Math.round(
          completedMath.reduce((sum, p) => sum + (p.score || 0), 0) /
            completedMath.length
        )
      : 0;

  const totalTimeSpent = mathProgress.reduce(
  ...
  const mathSkills: Skill[] = [
    {
      ...
      progress: Math.min(
        ...
        (completedMath.length /
          Math.max(
            1,
            mathAdventures.filter((a) =>
              a.skills.some(
                (s) =>
                  s.toLowerCase().includes('arithmetic') ||
                  s.toLowerCase().includes('addition') ||
                  s.toLowerCase().includes('subtraction')
              )
            ).length
          )) *
          100
      ...
      level:
        completedMath.length < 3
          ? 'beginner'
          : completedMath.length < 7
            ? 'intermediate'
            : 'advanced',
      ...
      lastPracticed: mathProgress[0]?.lastAccessed
        ? new Date(mathProgress[0].lastAccessed)
        : undefined,
    },
    {
      id: 'fractions',
      ...
      progress: Math.min(
        100,
        (completedMath.filter((p) => {
          const adv = mathAdventures.find((a) => a.id === p.adventureId);
          return adv?.skills.some(
            (s) =>
              s.toLowerCase().includes('fraction') ||
    ...
    },
    {
      ...
      progress: Math.min(
        100,
        (completedMath.length / Math.max(1, mathAdventures.length)) * 100
      ),
      level:
        completedMath.length < 5
          ? 'beginner'
          : completedMath.length < 10
            ? 'intermediate'
            : 'advanced',
        ...
        (completedMath.filter((p) => {
          ...
          return adv?.skills.some(
            (s) =>
              ...
              s.toLowerCase().includes('sequence')
          );
        }).length /
          Math.max(
            1,
  ...
  const learningPath: PathNode[] = [
    ...
    },
    {
      ...
      estimatedTime: 40,
      adventureId: mathAdventures.find((a) =>
        a.skills.some((s) => s.toLowerCase().includes('fraction'))
      )?.id,
