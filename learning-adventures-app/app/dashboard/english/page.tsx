...
function EnglishDashboardContent() {
  ...
  const englishAdventures = allAdventures.filter(
    (adv) => adv.category === 'english'
  );
  ...
  const completedEnglish = englishProgress.filter(
    (p) => p.status === 'COMPLETED'
  );
  const inProgressEnglish = englishProgress.filter(
    (p) => p.status === 'IN_PROGRESS'
  );

  const averageScore =
    completedEnglish.length > 0
      ? Math.round(
          completedEnglish.reduce((sum, p) => sum + (p.score || 0), 0) /
            completedEnglish.length
        )
      : 0;

  const totalTimeSpent = englishProgress.reduce(
  ...
  const englishSkills: Skill[] = [
    {
      ...
      progress: Math.min(
        100,
        (completedEnglish.filter((p) => {
          const adv = englishAdventures.find((a) => a.id === p.adventureId);
          return adv?.skills.some(
            (s) =>
              s.toLowerCase().includes('reading') ||
              s.toLowerCase().includes('comprehension')
          );
        }).length /
          Math.max(
            1,
            englishAdventures.filter((a) =>
              a.skills.some(
                (s) =>
                  s.toLowerCase().includes('reading') ||
                  s.toLowerCase().includes('comprehension')
      ...
      level:
        completedEnglish.length < 3
          ? 'beginner'
          : completedEnglish.length < 7
            ? 'intermediate'
            : 'advanced',
        ...
        (completedEnglish.filter((p) => {
          ...
          return adv?.skills.some(
            (s) =>
              ...
              s.toLowerCase().includes('punctuation')
          );
        }).length /
          Math.max(
            1,
    ...
    },
    {
      ...
      progress: Math.min(
        100,
        (completedEnglish.length / Math.max(1, englishAdventures.length)) * 100
      ),
      level: completedEnglish.length < 5 ? 'beginner' : 'intermediate',
  ...
  const learningPath: PathNode[] = [
    ...
    },
    {
      ...
      estimatedTime: 40,
      adventureId: englishAdventures.find((a) =>
        a.skills.some((s) => s.toLowerCase().includes('comprehension'))
      )?.id,
