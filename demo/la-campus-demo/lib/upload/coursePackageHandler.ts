...
export async function processCoursePackage(
  ...
  if (manifestEntry.header.size > 1024 * 1024) {
    throw new Error('metadata.json exceeds 1MB size limit');
  }
  ...
  for (const lessonMeta of manifest.lessons) {
    ...
    if (lessonEntry.header.size > 50 * 1024 * 1024) {
      throw new Error(`Lesson file exceeds 50MB size limit: ${lessonMeta.file}`);
    }
  ...
  if (manifest.thumbnail) {
    ...
    if (thumbnailEntry) {
      if (thumbnailEntry.header.size > 50 * 1024 * 1024) {
        throw new Error(`Thumbnail file exceeds 50MB size limit: ${manifest.thumbnail}`);
      }
