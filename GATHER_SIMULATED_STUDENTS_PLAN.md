...
**Goal**: Make the single-player browser demo _feel_ like a populated campus with other students, without building multiplayer.
...
- A campus "activity feed" widget (like a tiny corner ticker): _"Jordan finished the Algebra Challenge"_, _"Priya joined the Science Club"_
...
> "This feels like other kids are here with you — exploring, chatting, working on stuff — but it's entirely client-side. No server costs, no multiplayer bugs, no LLM API bills. When we're ready to add _real_ multiplayer, these NPCs become a fallback/filler layer so campus never feels empty even with low real user counts — exactly the same problem Erenshor solves for a solo player."
