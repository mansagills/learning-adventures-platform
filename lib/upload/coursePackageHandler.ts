...
export async function processCoursePackage(
  ...
  if (manifestEntry.header.size > 1048576) {
    throw new Error('Security Error: metadata.json is too large (exceeds 1MB limit).');
  }
  ...
  for (const lessonMeta of manifest.lessons) {
    ...
    if (lessonEntry.header.size > 50 * 1024 * 1024) {
      throw new Error(`Security Error: Lesson file ${lessonMeta.file} is too large (exceeds 50MB limit).`);
    }
  ...
  if (manifest.thumbnail) {
    ...
    if (thumbnailEntry) {
      if (thumbnailEntry.header.size > 50 * 1024 * 1024) {
        throw new Error(`Security Error: Thumbnail file ${manifest.thumbnail} is too large (exceeds 50MB limit).`);
      }
