...
export async function processCoursePackage(
  ...
  for (const lessonMeta of manifest.lessons) {
    ...
    if (lessonEntry.header.size > 100 * 1024 * 1024) {
      throw new Error(`Security Error: Lesson file ${lessonMeta.file} exceeds 100MB size limit`);
    }
  ...
  if (manifest.thumbnail) {
    ...
    if (thumbnailEntry) {
      if (thumbnailEntry.header.size > 10 * 1024 * 1024) {
        throw new Error(`Security Error: Thumbnail file ${manifest.thumbnail} exceeds 10MB size limit`);
      }
...
export function isCoursePackage(zip: AdmZip): boolean {
  ...
  if (manifest.header.size > 1024 * 1024) return false; // Prevent DoS on parsing large files
