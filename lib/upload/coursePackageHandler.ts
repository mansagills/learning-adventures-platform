...
export async function processCoursePackage(
  ...
  for (const lessonMeta of manifest.lessons) {
    ...
    if (lessonEntry.header.size > 50 * 1024 * 1024) {
      throw new Error(`Security Error: Lesson file exceeds 50MB size limit: ${lessonMeta.file}`);
    }
  ...
  if (manifest.thumbnail) {
    ...
    if (thumbnailEntry) {
      if (thumbnailEntry.header.size > 50 * 1024 * 1024) {
        throw new Error(`Security Error: Thumbnail file exceeds 50MB size limit: ${manifest.thumbnail}`);
      }
