...
export async function processGamePackage(
  ...
  // Security check: limit game file size to 50MB
  if (gameEntry.header && gameEntry.header.size > 50 * 1024 * 1024) {
    throw new Error(`Security Error: Game file ${manifest.gameFile} exceeds 50MB size limit`);
  }
