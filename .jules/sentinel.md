...
## 2025-02-28 - [Extract Metadata DoS]
**Vulnerability:** Unconstrained zip decompression when extracting `metadata.json` payloads in `app/api/internal/extract-metadata/route.ts`.
**Learning:** `adm-zip` buffers extracted content in memory when `getData()` is called. Without first bounding the uncompressed size via header checks, malicious zip bombs can consume excessive memory leading to server crash or slow-downs.
**Prevention:** Apply `assertArchiveWithinLimits` early in the `AdmZip` processing chain, and always use bounded extraction helpers (e.g. `readZipEntry` with `MAX_MANIFEST_BYTES`) rather than directly calling `.getData()` on entries.
